...
// ===== 1.3.91 破解延迟降质(Late-Crash 协议, OWASP MASTG 隐式响应) =====
// 设计依据(2026-09-01 调研): Themida《Protecting Better》第III-1章"延迟破解
// 检测"与 OWASP MASTG"Assessing Anti-RE Schemes"——立即报错/立即崩溃会把
// 检测点位置直接告诉攻击者, 使其可以定点 patch; 正确做法是"隐身响应":
// 检测到门禁不一致(补丁不完整)后不声不响, 随机延迟数分钟后进入降质态,
// 每 1.5~5.5 分钟出现一次极轻微症状(单帧快照不推进 = 一帧重复)。
// - 合法用户: gVcamGateClaim 一致, 标志永不置位, 渲染路径只多读一个
//   static BOOL —— 零开销, 不碰打光/帧率(帧率稳定是硬性验收标准)
// - 破解者: 只 patch 掉显眼门禁点(轮询线程的门禁派生)而漏掉散射点
//   (按键/加载/激活翻转时的独立复核)时, 延迟出现"偶发单帧卡一下"的
//   症状 —— 无法与检测点建立因果关联, AI 辅助破解的迭代节奏被打断
static BOOL gVcamGateClaim = NO;   // 主门禁声称有效(轮询线程每拍更新)
static BOOL gVcamDegradeOn = NO;   // 降质态(篡改确认后进入)
static double gVcamDegradeAt = 0;  // 降质生效时刻(篡改登记 + 随机延迟)
static double gVcamDegradeNext = 0; // 下次症状时刻

// 付费声明明文常量(467B, 有意不走字符串混淆层 —— strings/反编译/AI 辅助
// 分析二进制时直接可见): 数值初始化不受混淆器字面量变换影响。
// 同时兼任"诱饵数据": 破解者若删除此"看起来像许可数据"的常量, 校验失败
// 即触发降质 —— 删了反而中招, 留着则每个看到的人都读到付费声明。
static const uint8_t vcamPaidNote[467] = {
    ,
};
#define kVcamPaidNoteSum 62790u

// 篡改登记(跨文件全局; 供散射复核点调用): 首次登记后随机延迟 5~40 分钟
// 进入降质态。reason 仅入日志(logEnabled 时), 供正版授权诊断用。
BOOL vcamTamperNote(int reason) {
    CFAbsoluteTime now = CFAbsoluteTimeGetCurrent();
    if (gVcamDegradeOn || gVcamDegradeAt > 0) return NO;  // 幂等
    gVcamDegradeAt = now + 300.0 + (double)arc4random_uniform(2100);
    vcam_core_log([NSString stringWithFormat:@"[vcam] integrity note r=%d (deferred)", reason]);
    return YES;
}

// 轮询线程每拍推进(廉价): 篡改登记到期 → 进入降质态
static void vcamTamperTick(void) {
    if (gVcamDegradeAt > 0 && !gVcamDegradeOn
        && CFAbsoluteTimeGetCurrent() >= gVcamDegradeAt) {
        gVcamDegradeOn = YES;
        gVcamDegradeNext = CFAbsoluteTimeGetCurrent() + 60.0;
    }
}

// 散射复核(跨文件全局; 按键/视频加载/激活翻转等低频点调用):
// 主门禁声称"已激活"时, 用【独立于轮询线程的另一条代码路径】直读 plist +
// 直调验签(不经 vcamLicenseValid 缓存包装 —— 只 patch 显眼包装方法的破解
// 会漏掉本点)。复核失败 = 补丁不一致 → 登记篡改。未激活的合法用户
// gVcamGateClaim=NO 直接返回, 零开销。
// ECDSA ~14ms, 仅低频点调用, 不碰渲染/打光线程。
BOOL vcamScatterChk(int reason) {
    if (!gVcamGateClaim) return YES;
    ...
    if (![blob isKindOfClass:[NSString class]]
        || ![VCamNotify vcamLicenseVerifyBlob:blob]
        || ![VCamNotify vcamCrossDeviceCodeOK]) {
        vcamTamperNote(reason);
        return NO;
    }
    return YES;
}

// 付费声明完整性(init 期一次): 字节和被改/被删 → 登记篡改(诱饵性质)
static void vcamPaidNoteCheck(void) {
    uint32_t s = 0;
    for (size_t i = 0; i < sizeof(vcamPaidNote); i++) s += vcamPaidNote[i];
    if ((s & 0xFFFFu) != kVcamPaidNoteSum) vcamTamperNote(4);
}
...
- (void)renderReplacementToPixelBuffer:(CVPixelBufferRef)pixelBuffer pts:(double)pts {
    ...
    [_processLock lock];
    {
        ...
        // 1.3.91 破解降质症状点(仅篡改态生效): 到达症状时刻则跳过本拍快照推进
        // = 画面单帧重复一次(轻微), 间隔 90~330s 随机。合法用户 gVcamDegradeOn
        // 恒 NO, 仅一次 static BOOL 读 —— 帧率零扰动(硬性验收标准不受影响)。
        BOOL degradeBeat = NO;
        if (gVcamDegradeOn && nowQ >= gVcamDegradeNext) {
            gVcamDegradeNext = nowQ + 90.0 + (double)arc4random_uniform(240);
            degradeBeat = YES;
        }
        if (!degradeBeat && _liveYUVPixelBuffer &&
...
- (void)setEnabled:(BOOL)enabled {
    ...
    if (enabled) {
        ...
        // 1.3.91 散射复核点(激活翻转): 独立路径直调验签(14ms, 仅状态切换时)
        vcamScatterChk(2);
...
- (void)startStatePolling {
    ...
    _pollingActive = YES;
    // 1.3.91 付费声明完整性(init 期一次, 兼诱饵: 删改声明字节 → 延迟降质)
    vcamPaidNoteCheck();
    ...
    [[VCamNotify sharedInstance] startPollingWithInterval:0.15 callback:^(BOOL enabled) {
        ...
        // 1.3.91 延迟降质接线: 主门禁声称值(供散射复核点对照) + 状态机推进
        // (篡改登记后随机延迟到期才进入降质态 —— 攻击者无法建立因果关联)
        gVcamGateClaim = strongSelf->_licGate && strongSelf->_licMark;
        vcamTamperTick();
